...
from model.segmentor_scunet import EncoderDecoder
...
from train_fusion3 import Training 
...
def main():
    ...
    timeout=timedelta(seconds=8640000)
    ...
    # setting model ---------------------------------------------------------------------------
    # model = Fusion()
    model1 = EncoderDecoder(dim=48, mode='feature6', mode2='level123', resblock=True)
    model1 = DataParallel(model1.cuda(), device_ids=num_gpu)

    model2 = SalsaNextAdapter(nclasses)
    model2 = DataParallel(model2.to(device), device_ids=num_gpu)
    optimizer = Adam(model2.to(device).parameters())
    scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=100, eta_min=0.001)

    model1, _ = read_model(model1, creatergb_model, freeze=True)
    model2, optimizer = read_model(model2, segment_model, freeze=False, optimizer=optimizer)
    ...
    # setting data ----------------------------------------------------------------------------
    path = ''
    dataset = SemanticKITTI(data_path=path, shape=img_size, nclasses=nclasses, mode='train', front=False, split=True) # 360 
    ...
    t = Training(model1, model2, epochs, train_loader, val_loader, optimizer, criterion, 
